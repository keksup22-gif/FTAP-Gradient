...
-- ================================================================
-- AUTO-BRING / MAGNET (Module 4)
-- Attracts free props (Parts / Models without a Humanoid) to the
-- player and optionally pulls nearby players in. Every CFrame /
-- velocity manipulation is pcall-guarded with IsDescendantOf checks.
-- ================================================================

-- True if the part belongs to ANY player character (or any model that
-- has a Humanoid, e.g. ragdolls) - such parts are never magnetized.
local function belongsToAnyPlayer(part)
    local p = part
    while p do
...
-- Bring one free prop: velocity pull when far, snap to hand when near
local function magnetProp(part, root)
    if not isBasePart(part) then return end
    pcall(function()
        if not part.Parent or not part:IsDescendantOf(Workspace) then return end
        if part.Anchored then return end
        if part == HeldObject then return end -- never yank our own grab
        if belongsToAnyPlayer(part) then return end
        local rootPos = root.Position
        if typeof(rootPos) ~= "Vector3" then return end
        local dist = (part.Position - rootPos).Magnitude
        if dist > (Grabs_Config.MagnetRadius or 40) then return end

        claimNetworkOwnership(part)

        if dist > 8 then
            -- Attract: pull the prop toward the player
            local dir = (rootPos - part.Position).Unit
            part.AssemblyLinearVelocity = dir * (dist * 4)
        else
            -- Snap: teleport the prop to the player's hand area
            local target = rootPos + root.CFrame.LookVector * 4 + Vector3.new(0, 3, 0)
...
-- Bring one player: velocity pull toward our character
local function bringPlayer(player, root)
    if not player or player == LocalPlayer then return end
    local char = player.Character
    local target = char and char:FindFirstChild("HumanoidRootPart")
    if not isBasePart(target) then return end
    pcall(function()
        if not target.Parent or not target:IsDescendantOf(Workspace) then return end
        local rootPos = root.Position
        local dist = (target.Position - rootPos).Magnitude
        if dist > (Grabs_Config.MagnetRadius or 40) then return end
        if HeldObject and (target == HeldObject or target:IsDescendantOf(HeldObject)) then return end
        claimNetworkOwnership(target)
        local dir = (rootPos - target.Position).Unit
        target.AssemblyLinearVelocity = dir * 60
    end)
end

-- Magnet tick: throttled scan (0.1s), guarded end-to-end
local magnetTick = makeThrottled(0.1)
task.spawn(function()
    while true do
        task.wait(0.1)
        magnetTick(function()
            if not Grabs_Config.Magnet then return end
            local char = LocalPlayer.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if not isBasePart(root) then return end
            if not root.Parent or not root:IsDescendantOf(Workspace) then return end
            ...
            pcall(function()
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    ...
                    elseif obj:IsA("Model") and not obj:FindFirstChildOfClass("Humanoid") then
                        ...
                        if isBasePart(pp) then
                            magnetProp(pp, root)
                        else
                            -- Fallback: first BasePart in the model
                            for _, child in ipairs(obj:GetDescendants()) do
                                if child:IsA("BasePart") then
                                    magnetProp(child, root)
                                    break
                                end
                            end
                        end
            ...
            -- Bring players
            if Grabs_Config.BringPlayers then
                pcall(function()
                    for _, plr in ipairs(Players:GetPlayers()) do
                        bringPlayer(plr, root)
                    end
                end)
            end
        end)
...
-- ================================================================
-- TARGET & AURA SYSTEM (Module 5)
-- Anti-Fling Guard, Target ESP & Tracers, Target Selector, and
-- Auto-Aura (automatic prop bring + throw at closest target's HRP).
-- ================================================================

-- Anti-Fling Guard loop (Heartbeat)
RunService.Heartbeat:Connect(function()
    if not Grabs_Config.AntiFling then return end
    pcall(function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if isBasePart(root) and root.AssemblyLinearVelocity.Magnitude > 120 then
...
-- Target Selector: get closest valid enemy player
local function getClosestTarget()
    local closest, dist = nil, 9999
    local root = getRoot()
    if not isBasePart(root) then return nil end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            local tRoot = plr.Character:FindFirstChild("HumanoidRootPart")
            local tHum = plr.Character:FindFirstChildOfClass("Humanoid")
            if isBasePart(tRoot) and tHum and tHum.Health > 0 and tRoot:IsDescendantOf(Workspace) then
...
-- Target ESP Folder under CoreGui / PlayerGui
local targetEspFolder = Instance.new("Folder")
targetEspFolder.Name = "GradientTargetESP"
pcall(function()
    targetEspFolder.Parent = CoreGui
end)

-- Target ESP & Tracers renderer (RenderStepped)
local function updateTargetESP()
    targetEspFolder:ClearAllChildren()
    if not Grabs_Config.TargetESP then return end
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    local myRoot = getRoot()
    if not isBasePart(root) or not myRoot then return end
    ...
    pcall(function()
        ...
        box.Parent = targetEspFolder

        -- BillboardGui (Name, Distance, Health)
        local dist = math.floor((root.Position - myRoot.Position).Magnitude)
        local hp = math.floor(hum and hum.Health or 100)
        ...
        billboard.Parent = targetEspFolder
        ...
        hpLabel.Position = UDim2.fromOffset(0, 34)
        hpLabel.Text = "HP: " .. hp .. "/" .. math.floor(maxHp)
        ...
        hpBg.Size = UDim2.new(1, -12, 0, 3)
        hpBg.Position = UDim2.new(0, 6, 1, -4)
        ...
        -- Beam Tracer from Camera to Target
        local cam = Workspace.CurrentCamera
        if cam then
...
-- Auto-Aura / Auto-Throw loop: grabs nearest prop and hurls it at target HRP
local autoAuraTick = makeThrottled(0.3)
task.spawn(function()
    while true do
        task.wait(0.3)
        autoAuraTick(function()
            if not Grabs_Config.AutoAura then return end
            local char = LocalPlayer.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if not isBasePart(root) or not root:IsDescendantOf(Workspace) then return end

            local target = getClosestTarget()
            if not target or not target.Character then return end
            local tRoot = target.Character:FindFirstChild("HumanoidRootPart")
            if not isBasePart(tRoot) or not tRoot:IsDescendantOf(Workspace) then return end

            local distToTarget = (tRoot.Position - root.Position).Magnitude
            if distToTarget > (Grabs_Config.AuraRadius or 35) then return end

            -- Find nearest free prop within 30 studs
            local bestProp = nil
            local bestDist = 30
            pcall(function()
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    if isBasePart(obj) and not belongsToAnyPlayer(obj) and obj ~= HeldObject and not obj.Anchored then
            ...
            if bestProp and bestProp.Parent and bestProp:IsDescendantOf(Workspace) then
                pcall(function()
                    claimNetworkOwnership(bestProp)
                    bestProp.CFrame = root.CFrame + root.CFrame.LookVector * 3 + Vector3.new(0, 2, 0)
                    bestProp.AssemblyLinearVelocity = Vector3.zero
                    task.defer(function()
                        if bestProp and bestProp.Parent and bestProp:IsDescendantOf(Workspace) then
                            local dir = (tRoot.Position - bestProp.Position).Unit
                            applyThrowForce(bestProp, dir, 3500)
                        end
                    end)
                end)
            end
...
Tabs.Grabs:AddSection("Auto-Bring / Magnet (Module 4)")

local MagnetToggle = Tabs.Grabs:AddToggle("MagnetToggle", { Title = "Enable Magnet", Default = false })
MagnetToggle:OnChanged(function(Value) Grabs_Config.Magnet = Value end)

local BringPlayersToggle = Tabs.Grabs:AddToggle("BringPlayersToggle", { Title = "Bring Players", Default = false })
BringPlayersToggle:OnChanged(function(Value) Grabs_Config.BringPlayers = Value end)

Tabs.Grabs:AddSlider("MagnetRadiusSlider", {
    Title = "Magnet Radius",
    Default = 40,
    Min = 5,
    Max = 150,
    Rounding = 0,
    Callback = function(Value) Grabs_Config.MagnetRadius = Value end
})
...
Tabs.Grabs:AddSection("Target & Aura System (Module 5)")

local AntiFlingToggle = Tabs.Grabs:AddToggle("AntiFlingToggle", { Title = "Anti-Fling Guard", Default = false })
AntiFlingToggle:OnChanged(function(Value) Grabs_Config.AntiFling = Value end)

local TargetESPToggle = Tabs.Grabs:AddToggle("TargetESPToggle", { Title = "Target ESP & Tracers", Default = false })
TargetESPToggle:OnChanged(function(Value) Grabs_Config.TargetESP = Value end)

local AutoAuraToggle = Tabs.Grabs:AddToggle("AutoAuraToggle", { Title = "Auto-Aura (Prop Thrower)", Default = false })
AutoAuraToggle:OnChanged(function(Value) Grabs_Config.AutoAura = Value end)

Tabs.Grabs:AddSlider("AuraRadiusSlider", {
    Title = "Aura Radius",
    Default = 35,
    Min = 10,
    Max = 100,
    Rounding = 0,
    Callback = function(Value) Grabs_Config.AuraRadius = Value end
})
